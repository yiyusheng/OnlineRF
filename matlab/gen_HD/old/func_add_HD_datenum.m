function [hddn,smart,cost_hddn] = func_add_HD_datenum(dt,f_sort,predictorNames,sn_train,days_train,days_cost)
  %% if f_sort == 0 then keep original order else sort by 'sn_id' and 'datenum'
    ...
    id_valid_pos_train = unique(dt_pos.sn_id(ismember(dt_pos.sn_id,sn_train) & dt_pos.datenum >= days_train));  % find sn in pos+train and have days_train days data
    dt_valid_pos_train = dt_pos(ismember(dt_pos.sn_id,id_valid_pos_train) & dt_pos.datenum <= days_train,:);   % extract these sn with datenum from 1 to days_train
...
%   cost_hddn = func_eval_disk_time_order_bydatenum(dt_valid_pos_train,'datenum','hddn',days_cost,10,1);
    cost_hddn = 0;
