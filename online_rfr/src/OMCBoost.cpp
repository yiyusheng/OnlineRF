...
typedef enum {
    ORT, ORF, ORFR, OMCBOOST, OMCLPBOOST, LARANK
} CLASSIFIER_TYPE;
...
void help() {
    ...
    cout << "\t --orfr : \t use Online Random Forest (ORF) algorithm for regression." << endl;
...
int main(int argc, char *argv[]) {
    ...
    while (inputCounter < argc) {
        if (!strcmp(argv[inputCounter], "-h") || !strcmp(argv[inputCounter], "--help")) {
            ...
            classifier = ORF;
        } else if (!strcmp(argv[inputCounter], "--orfr")) {
            classifier = ORFR;
    ...
    case ORFR: {
        model = new OnlineRFR(hp, dataset_tr.m_numClasses, dataset_tr.m_numFeatures,
                             dataset_tr.m_minFeatRange, dataset_tr.m_maxFeatRange);
        break;
    }
    case OMCBOOST: {
